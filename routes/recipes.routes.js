...
router.get('/featured', async (req, res) => {
  try {
      // Fetch only featured food information
      const featuredFoodQuery = `
         SELECT 
          fi.id AS food_id,
          fi.food_name,
          fi.description,
      ...
      FROM food_information fi
      WHERE fi.recipe_featured = 1
      GROUP BY fi.id;
      `;

      const [featuredFoodResults] = await promisePool.query(featuredFoodQuery);

      if (featuredFoodResults.length === 0) {
          return res.status(404).json({ error: "No featured food found" });
      }
      ...
      // Format response
      const formattedResults = featuredFoodResults.map(food => ({
          id: food.food_id,
          food_name: food.food_name,
          description: food.description,
          serving_size: food.serving_size,
      ...
      console.error("Error fetching featured food information:", error);
      res.status(500).json({ error: "Database error" });
  }
});
    ...
    router.get('/popular', async (req, res) => {
      try {
        // Fetch the most liked & viewed recipes
        const popularQuery = `
          SELECT 
            fi.id AS food_id,
            fi.food_name,
          ...
          FROM food_information fi
          WHERE fi.views > 0 AND fi.likes > 0  -- Only get recipes with views & likes
          ORDER BY (fi.views + fi.likes) DESC  -- Sort by popularity
          LIMIT 10;
        `;
        ...
        const formattedResults = popularResults.map(food => ({
          ...
          nutritional_paragraph: food.nutritional_paragraph || null,
          author: food.author || null,
          recipe_featured: Boolean(food.recipe_featured),
          images: foodImagesMap[food.food_id] || []
        }));
  ...
  try {
    ...
    const formattedResult = {
      ...
      instructions: food.instructions
        ? food.instructions.split(' | ').map(instruction => instruction.trim())
        : [],
